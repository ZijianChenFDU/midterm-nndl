...
def fit_one_epoch(model, train_util, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}',mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen_val):
            ...
            with torch.no_grad():
                ...
                # pbar.set_postfix(**{'test_loss'  : val_loss / (iteration + 1)})
                pbar.update(1)

    print('Finish Validation')
    loss_history.append_loss(epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val)
    print('Epoch:'+ str(epoch + 1) + '/' + str(Epoch))
    print('Train Loss: %.3f || Test Loss: %.3f ' % (total_loss / epoch_step, val_loss / epoch_step_val))
